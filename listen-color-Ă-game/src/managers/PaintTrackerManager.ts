    ...
    finalizeAll() {
        ...
        this.shapeStates.forEach((state, _partId) => {
            ...
            if (isDone) {
                ...
                // → Đây là USER_ABANDONED thực sự (user bỏ giữa chừng khi đang vẽ)
                console.log(`[PaintTracker] finalizeAll: ${shapeId} ABANDONED (open attempt) → onQuit + finalize`);
                state.tracker.onQuit(Date.now());
                state.tracker.finalize();
            } else {
                // Shape chưa xong NHƯNG không có open attempt
                // (onDone đã gọi, pendingNewAttempt=true — vd: tô đỏ → tẩy → dừng lại)
                // → Không tạo attempt rỗng, chỉ finalize bình thường
                console.log(`[PaintTracker] finalizeAll: ${shapeId} incomplete but no open attempt → finalize only`);
                state.tracker.finalize();
            }
