...
export class Scene2UI {
    static createUI(scene: Phaser.Scene) {
        ...
        scene.add.image(charX, charY, TextureKeys.S2_Layer1).setOrigin(0.5, 0).setScale(boardScale * 0.9);

        // Letter U (Right)
        const uwX = cx + boardRight.displayWidth * 0.25;
        const uwY = boardY + boardRight.displayHeight * 0.15;
        scene.add.image(uwX, uwY, TextureKeys.S2_UW).setOrigin(0.5, 0).setScale(boardScale * 0.9);
