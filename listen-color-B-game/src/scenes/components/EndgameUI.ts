...
export class EndgameUI {
    static createCompleteUI(scene: Phaser.Scene, onRestart: () => void, onExit: () => void): Phaser.GameObjects.Container {
        const container = scene.add.container(0, 0).setDepth(100);

        this.createBackground(scene, container);
        this.createAnimatedIcon(scene, container);
        this.createButtons(scene, onRestart, onExit, container);

        return container;
    }

    static createBackground(scene: Phaser.Scene, container?: Phaser.GameObjects.Container) {
        const w = scene.scale.width;
        const h = scene.scale.height;

        // Banner
        const banner = scene.add.image(w / 2, h / 2 - h * 0.12, 'banner_congrat')
            .setOrigin(0.5)
            .setDisplaySize(w * 0.9, h * 0.9);

        if (container) container.add(banner);
        else banner.setDepth(100);
    }

    static createAnimatedIcon(scene: Phaser.Scene, container?: Phaser.GameObjects.Container) {
        const w = scene.scale.width;
        const h = scene.scale.height;
        ...
        if (scene.textures.exists('icon')) {
            ...
            if (container) container.add(icon);
            else icon.setDepth(1005);
    ...
    static createButtons(scene: Phaser.Scene, onRestart: () => void, onExit: () => void, container?: Phaser.GameObjects.Container) {
        ...
        if (container) {
            container.add([replayBtn, exitBtn]);
        } else {
